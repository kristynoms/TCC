...
function get_twitter_search_results($query,$lang='',$result_type='',$geo='')
{
   ...
   if (isset($json) && !empty($json)){
       if(!empty($json->errors)){
        ...
        <div class="tweet_name"><strong>'.$tweet->user->name.'</strong> <small>@'.$tweet->user->screen_name.'</small><br />'. $tweet_text.'<br /><img src="'.constant("TWITTER_API_PKG_DIR").'twitter_api_pkg/icons/reply_arrow.png" border="0" onerror="this.style.display=\'none\'"> <small><a href="https://twitter.com/intent/tweet?in_reply_to='.$tweet->id.'" target="_blank">Reply</a></small> <img src="'.constant("TWITTER_API_PKG_DIR").'twitter_api_pkg/icons/retweet_arrows.png" border="0" onerror="this.style.display=\'none\'"> <small><a href="https://twitter.com/intent/retweet?tweet_id='.$tweet->id.'" target="_blank">Retweet</a></small> <img src="'.constant("TWITTER_API_PKG_DIR").'twitter_api_pkg/icons/favorite_star.png" border="0" onerror="this.style.display=\'none\'"> <small><a href="https://twitter.com/intent/favorite?tweet_id='.$tweet->id.'" target="_blank">Favorite</a></small><br /><small>'. $time.', '. $date.'</small> <small><a href="https://twitter.com/'.$tweet->user->screen_name.'/status/'.$tweet->id.'" target="_blank">Expand</a></small></div>
                        </div>';	
	               }
	               $i++;			
	           endforeach;
...
function twitter_search_php($twitter_search_query,$lang='',$result_type='',$geo='')
{
   ...
   */
   if($twitter_activity != ''){
       ...
       $show_twitter_activity .= '<div><strong>Twitter search results for: </strong> '.$twitter_search_query.'</div>';
       $show_twitter_activity .= '<div id="tweets">'. $twitter_activity.'</div>';
       $show_twitter_activity .= '<div style="clear:both;"></div></div>';
   }else{
       $show_twitter_activity = '<div id="twitter_search_results"><div id="tweets">We were unable to retrieve Twitter search results for the term of '.$twitter_search_query.'</div><div style="clear:both;"></div></div>';
   }
...
function twitter_generate_search_form($url,$show_lang,$result_type,$show_distance,$showmap,$get=''){
   ...
   */
   $the_form = '<form action="'.$url.'" method="get">
               Search Term: <input type="text" name="twitter_keywords" value="'.htmlentities($keywords).'" />';
   if($show_lang == true){
   /**
   ...
   */
   if($result_type == true){
       $the_form .= str_replace('value="'.$result_type.'"','value="'.$result_type.'" selected',' Sort Order: <select id="result_type" name="result_type">
   ...
   */
    if($show_distance == true && $showmap == true){
        $the_form .= '<div id="map_holder">
                      <div id="map_help">To find the latitude and longitude of a place, use the search box below.</div>
                      <div id="panel">
   ...
   */	  
   if($show_distance == true){		  
       $the_form .= '<div id="location">Latitude: <input type="text" id="lat" name="lat" value="'.$lat.'" size="20" onclick="changeSort();" /> Longitude: <input type="text" id="lon" name="lon" value="'.$lon.'" size="20" onclick="changeSort();" /> Distance: <input type="text" name="distance" value="'.(($distance == '')?100:$distance).'" size="10" /> <select name="mikm">';
       $the_form .= str_replace('value="'.$mikm.'"','value="'.$mikm.'" selected','<option value="mi">Miles</option>
           <option value="km">Kilometers</option>
          </select></div>');
   }
		  
   $the_form .= '<button class="twitter_search" type="submit">Search</button>
                </form>';
